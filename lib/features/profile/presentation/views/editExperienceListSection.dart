...
class EditExperienceSection extends StatefulWidget {
  const EditExperienceSection({super.key});

  @override
  State<EditExperienceSection> createState() => _EditExperienceSectionState();
}

class _EditExperienceSectionState extends State<EditExperienceSection> with WidgetsBindingObserver {
  late ScrollController scrollController;

  late bool _hideSaveButton;

  List<WorkingExperienceModel> modelList = [];
  List<TextEditingController> positionHeldControllers = [];
  List<TextEditingController> employerControllers = [];
  ...
  void addDates(WorkingExperienceModel? item) {
    if (item != null) {
  ...
  void addControllers(WorkingExperienceModel? item) {
    positionHeldControllers.add(addPositionHeldController(item));
    employerControllers.add(addEmployerController(item));
  ...
  TextEditingController addPositionHeldController(WorkingExperienceModel? item) {
    if (item != null) {
  ...
  TextEditingController addEmployerController(WorkingExperienceModel? item) {
    if (item != null) {
      return TextEditingController(text: item.employer);
    } else {
      return TextEditingController();
    }
  }
  
  TextEditingController addLocationController(WorkingExperienceModel? item) {
    if (item != null) {
  ...
  TextEditingController addStartDateController(WorkingExperienceModel? item) {
    if (item != null) {
  ...
  TextEditingController addEndDateController(WorkingExperienceModel? item) {
    if (item != null) {
  ...
  void removeEmployerController(int index) {
    employerControllers[index].dispose();
    employerControllers.removeAt(index);
  }
      ...
      entries.add(
        Padding(
          ...
          child: ExperienceForm(
            ...
            employerController: employerControllers[x], 
      ...
      children: [
        Scaffold(
          appBar: SubPageAppBar(
            ...
            title: "Edit Working Experiences",
          ),
          body: SafeArea(
            child: Stack(
              children: [
                        ...
                        onTap: () async {
                          if (formKey.currentState!.validate()) {
                            ...
                            for (int x = 0; x < modelList.length; x++) {
                              ...
                              modelList[x].employer = employerControllers[x].text.trim();
                              modelList[x].location = locationControllers[x].text.trim();
                              modelList[x].startDate = startDates[x]!;
                              modelList[x].endDate = endDates[x]!;
                            }
                            bool successfulUpload = await profileProvider.setWorkingExperiences(
                              modelList
                            );
